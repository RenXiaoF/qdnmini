...
Page({
    data: {
        ...
        prom_info: null,     // 商品订单促销列表信息
        prom_price: null,    // 商品的价格信息（会员价，抢购价，团购价，促销价）
        ...
        timer: null, //活动倒计时定时器
        spec_img: '', // 商品小图标
        is_visible: "block", // 商品详情的短视频是否显示播放
    },

    onReady: function (res) {
        // 短视频播放组件
        this.video_content = wx.createVideoContext('myVideo');
    },
    ...
    onLoad: function (options) {
        ...
        request.get('/api/goods/goodsInfo', {
            ...
            success: function(res) {
                ...
                that.setData({ result: res.data.result, cartGoodsNum: res.data.result.cart_list_count});
    ...
    initData: function (data) {
        ...
        // 商品促销活动的倒计时
        this.initActivity(data.prom_price);
        //初始化规格
    ...
    initSpecsPrice: function (data) {
        ...
        // 设置规格小图的默认值
        this.data.spec_img = data.goods.original_img;
    ...
    initActivity: function (activity) {
        if (activity.prom_type && (activity.prom_type == 1 || activity.prom_type == 2 || activity.prom_type == 3)) {
        ...
        this.data.select.activity = activity;
    ...
    initSelectedData: function () {
        ...
        var stock = 0;
        var price = 0;
        var specImg = this.data.data.goods.original_img;
    ...
    createActivityTimer: function () {
        ...
        this.data.timer = setInterval(function () {
            ...
            that.data.select.activity.countTime = remainTime;
    ...
    selectSpec: function (e) {
        ...
        var specIdx = e.currentTarget.dataset.specidx;
        ...
        // 设置规格小图标
        var src = list[listIdx].spec_list[specIdx].src;
        if (src) {
            this.data.spec_img = src;
            this.setData({spec_img: this.data.spec_img});
        }

        this.data.specSelect = newSpecSelect;
        this.setData({
    ...
    addCart: function (e) {
        ...
        if (e.currentTarget.dataset.action == 'add') { //加入购物车
            ...
            request.post('/api/cart/wechat_addCart', {
                data: data,
                success: function (res) {
    ...
    bindPlay: function() {
        ...
        this.data.is_visible = 'none';
        this.setData({is_visible: this.data.is_visible});
    },

    /** 短视频暂停 */
    bindPause: function() {
        this.video_content.pause();
        this.data.is_visible = 'block';
        this.setData({is_visible: this.data.is_visible});
    },
