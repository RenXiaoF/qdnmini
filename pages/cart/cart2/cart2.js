...
Page({
            ...
            if (conponUse !== '') {
                ...
                for (var i = 0; i < stores.length; i++) {
                    if (stores[i].store_id == conponUse.storeId) {
                        ...
                        this.data.formData.lid = conponUse.useCouponListId;
                        this.data.formData.store_id = conponUse.storeId;
    ...
    requestCart2: function (addressId) {
        ...
        request.get('/api/cart/cart2', {
            ...
            success: function (res) {
                ...
                that.data.shipping = res.data.result.shipping;
                that.setData({shipping: that.data.shipping});
                that.data.coupon_list = res.data.result.couponNum;
                that.setData({coupon_list: that.data.coupon_list});
                that.data.formData.shipping_code = res.data.result.shipping[0].code;
    ...
    calculatePrice: function (formData, submitOrder) {
        ...
        // 获取使用的优惠券信息
        var coupon = that.getCouponCart3();
        ...
        if (this.data.goods.action){
            postData = {
                ...
                coupon_id: JSON.stringify(coupon),
            };
        }else{
            postData = {
                address_id: that.data.order.addressList.address_id,
        ...
        request.post('/api/cart/cart3', {
            ...
            success: function (res) {
                ...
                if (that.data.orderPrices.payables <= 0) {
    ...
    useCoupon: function (e) {
        var store = this.data.coupon_list.coupon_count[e.currentTarget.dataset.idx];
        var num = store.store_coupon_count;
        var lid = e.currentTarget.dataset.couponid;
    ...
    getCouponCart3: function() {
        var carts = this.data.order.storeShippingCartList;

        var coupon = [];
        carts.forEach(function(cart){
            if (cart.useCouponListId) {
                coupon.push({'store_id': cart.store_id, 'coupon_id': cart.useCouponListId});
            }
        });
